/**
* Scavenger hunts plugin. Only works in a room with the id 'gambling'.
...
exports.commands = {
	...
	scavengers: function (target, room, user) {
		return this.parse('/join gambling');
	},
	...
	starthunt: function (target, room, user, connection, cmd) {
		...
		Rooms.rooms.gambling.addRaw('<div class="broadcast-blue"><strong>' + result + '</strong></div>');
	},
	...
	resethunt: function (target, room, user) {
		...
		if (result) {
			...
			Rooms.rooms.gambling.addRaw('<div class="broadcast-blue"><strong>The Scavenger Hunt was reset by <em>' + Tools.escapeHTML(user.name) + '</em>.</strong></div>');
		}
		result = null;
	},
	...
	scavengerhelp: function (target, room, user) {
		...
		this.sendReplyBox(
			...
			'- /scavengers - Join the Gambling room<br />' +
